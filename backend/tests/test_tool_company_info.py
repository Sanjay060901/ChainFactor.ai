...
class TestCompanyInfoMockRules:
    ...
    def test_maharashtra_27_returns_active_2015(self):
        ...
        result = get_company_info(company_gstin="27AABCT1234R1ZM", _demo=False)
    ...
    def test_karnataka_29_returns_active_2018(self):
        ...
        result = get_company_info(company_gstin="29AABCW5678R1ZX", _demo=False)
        assert result["status"] == "active"
    ...
    def test_up_09_returns_active_2020(self):
        ...
        result = get_company_info(company_gstin="09AAACR5678R1ZQ", _demo=False)
        assert result["status"] == "active"
    ...
    def test_delhi_07_returns_dormant(self):
        ...
        result = get_company_info(company_gstin="07AABCD1234R1ZP", _demo=False)
    ...
    def test_unknown_prefix_returns_default(self):
        ...
        result = get_company_info(company_gstin="33AABCT9999R1ZZ", _demo=False)
        assert result["status"] == "active"
    ...
    def test_another_unknown_prefix(self):
        ...
        result = get_company_info(company_gstin="06XYZAB1234R1ZZ", _demo=False)
        assert result["status"] == "active"
        assert result["incorporated"] == "2019"
        assert result["paid_up_capital"] == 25_000_000.0
...
class TestCompanyInfoDemoMode:
    """DEMO_MODE always returns active company regardless of GSTIN."""

    def test_demo_mode_returns_active(self):
        """Demo mode returns active company even for unknown GSTIN prefix."""
        result = get_company_info(company_gstin="33AABCT9999R1ZZ", _demo=True)
        assert result["status"] == "active"
        assert result["incorporated"] == "2015"
        assert result["paid_up_capital"] == 100_000_000.0

    def test_demo_mode_overrides_dormant(self):
        """Demo mode overrides dormant status with active."""
        result = get_company_info(company_gstin="07AABCD1234R1ZP", _demo=True)
        assert result["status"] == "active"
...
class TestCompanyInfoEdgeCases:
    ...
    def test_empty_gstin_returns_default(self):
        ...
        result = get_company_info(company_gstin="", _demo=False)
        assert result["status"] == "active"
        assert result["incorporated"] == "2019"

    def test_short_gstin_returns_default(self):
        """Single-char GSTIN should fallback to default profile."""
        result = get_company_info(company_gstin="2", _demo=False)
        assert result["status"] == "active"
